...
# ============================================================================
# FULL-SCREEN PROCEDURAL CRYPT BACKGROUND (reusable across menu screens)
# ============================================================================
# Originally inlined in SelectPlayer.gd:_draw() lines 161-226; extracted here
# so ConfigJoy, SelectPlayer, and any future menu screen can share the same
# fantasy crypt/ruderi look with a single call.
#
# Usage from any CanvasItem (Control or Node2D) _draw():
#   EnvironmentArt.draw_crypt_background(self, size, anim_time)
#
# Layers (drawn in order, back-to-front):
#   1. Night gradient (deep purple → midnight blue)
#   2. Animated bottom fog (5 pulsing circles)
#   3. Stone crypt columns (left + right) with fluting, cracks, moss
#   4. Stone arch at the top (24-segment semicircle)
#   5. Decorative skulls on the sides (uses get_skull_texture)
#   6. Animated torches (left + right) with flickering 3-layer flame
#   7. Rubble piles on the floor
#   8. Edge vignette for depth
func draw_crypt_background(canvas_item: CanvasItem, vp_size: Vector2,
                anim_time: float) -> void:
        ...
        for i in 32:
                ...
                canvas_item.draw_rect(Rect2(0, i * band_h, vp_size.x, band_h),
        ...
        for i in 5:
                ...
                var fog_col: Color = Color(0.15, 0.12, 0.25,
                        fog_alpha * (1.0 - float(i) * 0.15))
                canvas_item.draw_circle(Vector2(cx + sin(anim_time * 0.5 + i) * 80, fog_y),
                        fog_r, fog_col)

        # 3. Colonne di pietra laterali (stile cripta D&D)
        _draw_crypt_column_on(canvas_item, vp_size.x * 0.08, vp_size.y * 0.15, 0.7)
        _draw_crypt_column_on(canvas_item, vp_size.x * 0.92, vp_size.y * 0.15, 0.7)
        ...
        for i in 24:
                ...
                canvas_item.draw_circle(Vector2(ax, ay), 8.0, Color(0.15, 0.12, 0.10))

        # 5. Teschi decorativi sui lati
        var skull_tex: Texture2D = get_skull_texture()
        if skull_tex:
                canvas_item.draw_texture_rect(skull_tex,
        ...
        _draw_torch_flame_on(canvas_item, vp_size.x * 0.12, vp_size.y * 0.25, anim_time)
        _draw_torch_flame_on(canvas_item, vp_size.x * 0.88, vp_size.y * 0.25, anim_time)
        ...
        for i in 6:
                ...
                canvas_item.draw_circle(Vector2(rubble_x, rubble_y),
                        12.0 + float(i % 3) * 4.0, rubble_col)
                canvas_item.draw_circle(Vector2(rubble_x + 8, rubble_y - 4),
                        6.0, rubble_col.darkened(0.3))

        # 8. Vignette scuro ai bordi per profondita'
        for i in 10:
                var vign_alpha: float = 0.05 * (10 - i) / 10.0
                canvas_item.draw_rect(Rect2(0, 0, float(i) * 4.0, vp_size.y),
                        Color(0, 0, 0, vign_alpha), true)
                canvas_item.draw_rect(Rect2(vp_size.x - float(i) * 4.0, 0, 4.0, vp_size.y),
                        Color(0, 0, 0, vign_alpha), true)


# --- Helpers per il cripta background (privati) ---

# Colonna di pietra con capitello, scanalature, crepe e muschio.
func _draw_crypt_column_on(ci: CanvasItem, x: float, y: float, scale_val: float) -> void:
        ...
        # Base (piu' larga)
        ci.draw_rect(Rect2(x - col_w / 2.0 - 5, y + col_h - 10, col_w + 10, 20), rock_dark)
        # Fusto
        ci.draw_rect(Rect2(x - col_w / 2.0, y, col_w, col_h), rock_mid)
        # Scanalature
        for i in 3:
                var sx: float = x - col_w / 2.0 + float(i + 1) * col_w / 4.0
                ci.draw_rect(Rect2(sx - 1, y, 2, col_h), rock_dark)
        # Highlight sinistro (luce da torce)
        ci.draw_rect(Rect2(x - col_w / 2.0, y, 3, col_h), rock_light)
        # Capitello (parte superiore)
        ci.draw_rect(Rect2(x - col_w / 2.0 - 5, y - 10, col_w + 10, 15), rock_dark)
        ci.draw_rect(Rect2(x - col_w / 2.0 - 3, y - 15, col_w + 6, 8), rock_mid)
        # Crepe
        ci.draw_line(Vector2(x + 5, y + 50), Vector2(x + 8, y + 200),
                Color(0.05, 0.04, 0.03), 1)
        ci.draw_line(Vector2(x - 10, y + 100), Vector2(x - 7, y + 250),
                Color(0.05, 0.04, 0.03), 1)
        # Muschio alla base
        ci.draw_rect(Rect2(x - col_w / 2.0, y + col_h - 8, col_w, 6),
                Color(0.12, 0.22, 0.08))
        for i in 4:
                ci.draw_circle(Vector2(x - 15 + float(i) * 10, y + col_h - 5),
                        2.0, Color(0.15, 0.28, 0.10))


# Torcia con fiamma animata.
func _draw_torch_flame_on(ci: CanvasItem, x: float, y: float, anim_time: float) -> void:
        var flicker: float = sin(anim_time * 18.0) * 1.5
        var flicker2: float = cos(anim_time * 22.0) * 1.0
        # Supporto metallico
        ci.draw_rect(Rect2(x - 2, y, 4, 15), Color(0.25, 0.22, 0.20))
        # Aura
        ci.draw_circle(Vector2(x, y), 16.0, Color(1.0, 0.6, 0.2, 0.15))
        ci.draw_circle(Vector2(x, y), 10.0, Color(1.0, 0.5, 0.1, 0.25))
        # Fiamma esterna (arancione)
        ci.draw_circle(Vector2(x, y - 5), 6.0 + flicker, Color(1.0, 0.4, 0.0, 0.8))
        # Fiamma media (gialla)
        ci.draw_circle(Vector2(x, y - 5), 4.0 + flicker * 0.6, Color(1.0, 0.8, 0.2, 0.9))
        # Nucleo (bianco)
        ci.draw_circle(Vector2(x, y - 5), 2.0 + flicker2 * 0.5, Color(1.0, 1.0, 0.8, 1.0))
        # Scintille
        for i in 3:
                var spark_y: float = y - 10.0 - float(i) * 5.0 + sin(anim_time * 3.0 + i) * 2.0
                var spark_x: float = x + cos(anim_time * 4.0 + i * 2.0) * 3.0
                ci.draw_circle(Vector2(spark_x, spark_y), 1.0, Color(1.0, 0.7, 0.2, 0.6))
