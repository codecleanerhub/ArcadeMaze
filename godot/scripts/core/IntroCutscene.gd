...
#
# FIX (stile geroglifici): le scritte usano uno stile "scolpito su pietra"
# con outline color pietra + fill dorato. Custom _draw invece di Label.
...
func _draw() -> void:
	...
	var panel_y: float = size.y * 0.72
	var panel_h: float = size.y * 0.25
	var panel_x: float = size.x * 0.08
	var panel_w: float = size.x * 0.84
	# Sfondo pietra scuro
	draw_rect(Rect2(panel_x, panel_y, panel_w, panel_h),
		Color(0.12, 0.08, 0.04, 0.85), true)
	# Bordo pietra dorato
	draw_rect(Rect2(panel_x, panel_y, panel_w, panel_h),
		Color(0.6, 0.5, 0.2, 0.7), false, 2.0)
	# Bordo interno
	draw_rect(Rect2(panel_x + 4, panel_y + 4, panel_w - 8, panel_h - 8),
	...
	for line_idx in lines.size():
		...
		var line_x: float = (size.x - line_w) * 0.5
		...
		for off in [Vector2(-2, 2), Vector2(2, 2), Vector2(0, 3)]:
			draw_string(font, Vector2(line_x + off.x, ly + off.y), line,
		...
		draw_string(font, Vector2(line_x, ly), line,
	...
	var prog_size: int = 20
	var prog_y: float = 30.0
	# Sfondo piccolo
	draw_rect(Rect2(15, 15, 80, 30), Color(0.12, 0.08, 0.04, 0.85), true)
	draw_rect(Rect2(15, 15, 80, 30), Color(0.6, 0.5, 0.2, 0.6), false, 1.0)
	# Outline
	for off in [Vector2(-1, 0), Vector2(1, 0), Vector2(0, -1), Vector2(0, 1)]:
		draw_string(font, Vector2(25 + off.x, prog_y + off.y), prog_text,
			HORIZONTAL_ALIGNMENT_LEFT, -1, prog_size, STONE_COLOR)
	draw_string(font, Vector2(25, prog_y), prog_text,
	...
	var skip_size: int = 18
	var skip_w: float = font.get_string_size(skip_text, HORIZONTAL_ALIGNMENT_RIGHT, -1, skip_size).x
	var skip_x: float = size.x - skip_w - 30
	var skip_y: float = size.y - 20
	# Outline pietra
	for off in [Vector2(-1, 0), Vector2(1, 0), Vector2(0, -1), Vector2(0, 1)]:
		draw_string(font, Vector2(skip_x + off.x, skip_y + off.y), skip_text,
			HORIZONTAL_ALIGNMENT_LEFT, -1, skip_size,
			Color(STONE_COLOR.r, STONE_COLOR.g, STONE_COLOR.b, skip_alpha))
	draw_string(font, Vector2(skip_x, skip_y), skip_text,
		HORIZONTAL_ALIGNMENT_LEFT, -1, skip_size,
