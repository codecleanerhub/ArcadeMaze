...
func _build_ui() -> void:
	...
	_pause_label.position = Vector2(0, -20)
	_pause_label.size = Vector2(size.x, 80)
	add_child(_pause_label)

	# --- "PRESS P TO RESUME" label (smaller, below) ---
	_resume_label = Label.new()
	_resume_label.text = "PRESS P TO RESUME"
	_resume_label.set_anchors_preset(Control.PRESET_CENTER)
	...
	_resume_label.position = Vector2(0, 60)
	_resume_label.size = Vector2(size.x, 30)
